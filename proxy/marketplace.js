...
// VS Code Marketplace color-theme fetcher (proxy-side).
//
// Ported verbatim from hermes-agent's Electron main process
// (apps/desktop/electron/vscode-marketplace.ts) so the browser renderer gets
// the same capability without CORS: the proxy does the HTTPS work and hands
// back the raw theme JSON text. No theme code is ever executed — we only read
// `package.json` + the referenced `*.json` theme files out of the archive.
//
// Dependency-free on purpose: a `.vsix` is a plain zip, so we parse the
// central directory and inflate just the entries we need with `zlib`.
...
async function fetchMarketplaceThemes(id) {
  ...
  const vsix = await request(vsixUrl, { headers: { 'User-Agent': 'Hermes-Desktop-Web' } });
